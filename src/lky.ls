...
import_code('/root/LuckyScript/argparse/argparse.a');
import_code('/root/foxlib/logger.a');
logger=Logger;
//public={};
import_code('/root/LuckyScript/lky/lib/lexer.a');
import_code('/root/LuckyScript/lky/lib/parser.a');
import_code('/root/LuckyScript/lky/lib/transpiler.a');
...
LS.transpile(IFile,OFile,Importable=false,Clean=false)={
    ...
    if !tree {
        err = parser.error();
        LS.err('Parser',LS.getLine(content,err.first.pos),content,err.msg,IFile);
        exit()
    };

    transpiled = Transpile(tree);
