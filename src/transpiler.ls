...
Transpile(node,ind=0,alt=0)={
    ...
    if type=='StateList' {
        ...
        s+='for '+Transpile(node.init,ind+1).trim()+' in '+Transpile(node.item,ind+1).trim();
        s+='\n'+Transpile(node.blk,ind+1);
        return s+'\n'+(char(9)*ind)+'end for'
        ...
        return s+'new '+Transpile(node.obj,ind+1).trim()
        ...
        if op=='Assign' && node.l.classID=='FuncAssign' {
        ...
        for part in node.parts {
            if __part_idx!=0 s+=',';
            s+=Transpile(part.key,ind+1).trim()+':'+Transpile(part.value,ind+1).trim();
        };
        return s+'}'
    } else if type=='Constructor' {
        s+=Transpile(node.id,ind+1).trim()+'='+Transpile(node.default,ind+1).trim();
        return s
    } else if type=='FString' {
        for i in range(0, node.elems.len-1) {
            s+='"'+node.elems[i]+'"';
            if i < node.exprs.len s+='+str('+Transpile(node.exprs[i],ind+1).trim()+')+'
        };
        return s
    } else {
        return ''
